...
class SuffixSum():
    ...
    def remove(self, val):
        for i in range(33):
            self.sums[i] -= val>>i
        self.length -= 1
    def bad(self, bad_count):
        return self.sums[min(bad_count,32)]
    def good(self, bad_count):
        return self.sums[min(bad_count,32)] - self.price
...
def main():
    ...
    for _ in range(TestCases):
        ...
        # ans = -INF
        # sm = 0
        # for goodcnt in range(n):
        #     cur = arr[goodcnt]
        #     badcnt = n - goodcnt
        #     cost = goodcnt * k
        #     ans = max(ans, sm + sf.bad(badcnt+1) - cost)
        #     sm += cur
        #     sf.remove(cur)
        # goodcnt, badcnt = n, 0
        # cost = goodcnt * k
        ...
        for badcnt in range(n+1):
            sm, j = 0,1
            for i in range(n-badcnt, n-badcnt + min(33, badcnt)):
                sm += arr[i]>>j
                j += 1
            ans = max((ps[n-badcnt] + sm - (n - badcnt) * k), ans)
        print(ans)
