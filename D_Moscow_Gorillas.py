...
def main():
    ...
    for _ in range(TestCases):
        ...
        n = int(input())
        arr1 = [int(i) - 1 for i in input().split()]
        arr2 = [int(i) - 1 for i in input().split()]
        # s = input()
        ...
        ans += nC2(min(st1, st2))
        ans += nC2(max(st1, st2) - min(st1, st2) - 1)
        ans += nC2(n - max(st1, st2) - 1)

        for cur in range(n - 1):
            st1 = min(st1, ind1[cur])
            en1 = max(en1, ind1[cur])
            st2 = min(st2, ind2[cur])
            en2 = max(en2, ind2[cur])

            mex = cur + 1
            i1, i2 = ind1[mex], ind2[mex]
            l1 = range(0, st1 + 1)
            r1 = range(en1, n)
            l2 = range(0, st2 + 1)
            r2 = range(en2, n)
            ...
            if i1 in l1:
                l1 = range(i1 + 1, l1.stop)
            elif i1 in r1:
                r1 = range(r1.start, i1)
            else:
                continue

            if i2 in l2:
                l2 = range(i2 + 1, l2.stop)
            elif i2 in r2:
                r2 = range(r2.start, i2)
            else:
                continue

            l = rangeIntersection(l1, l2)
            r = rangeIntersection(r1, r2)

            ans += max(0, l.stop - l.start) * max(0, r.stop - r.start)
