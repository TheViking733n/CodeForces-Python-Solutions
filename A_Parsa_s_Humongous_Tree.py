...
# def dfs(node, pa, graph, v, l, r, ans):
#     if v[node] == -1:
#         if len(graph[node]) == 1:
#             par = graph[node][0]
#             val1 = abs(l[par] - r[node])
#             val2 = abs(r[par] - l[node])
#             if val1 < val2:
#                 v[node] = l[node]
#             else:
#                 v[node] = r[node]
...
#                 if ch == pa:
#                     continue
#                 dfs(ch, node, graph, v, l, r, ans)
#                 val1 += abs(v[ch] - r[node])
#                 val2 += abs(v[ch] - l[node])
#             if val1 < val2:
#                 v[node] = l[node]
#             else:
#                 v[node] = r[node]
...
@bootstrap
def dfs(node, pa, graph, l, r):
    ans1 = ans2 = 0
    for ch in graph[node]:
        if ch == pa: continue
        v1, v2 = yield dfs(ch, node, graph, l, r)
        ans1 += max(abs(l[node]-l[ch])+v1, abs(l[node]-r[ch])+v2)
        ans2 += max(abs(r[node]-l[ch])+v1, abs(r[node]-r[ch])+v2)
    yield ans1, ans2
