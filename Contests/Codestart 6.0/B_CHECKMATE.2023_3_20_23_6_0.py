...
def getDanger(chess):
    ...
    for _ in range(TestCases):
        ...
        for i in range(8):
            for j in range(8):
                danger[i][j] = int(danger[i][j])
            
            
        for x, y in king + rook + bishop + knight + queen:
            # dsu.union(8 * x + y, 8 * myking[0] + myking[1])
            danger[x][y] = 2


        return danger, myking
        ...
        for x in dx:
            for y in dy:
                if 0 <= x + myking[0] < 8 and 0 <= y + myking[1] < 8:
                    i, j = x + myking[0], y + myking[1]
    ...
    for x in dx:
        for y in dy:
            if 0 <= x + myking[0] < 8 and 0 <= y + myking[1] < 8:
                ...
                # if dsu.set_size(8 * i + j) == 1:
                if danger[i][j] != 1:
                    oldChess = copy2d(chess)
                    chess[i][j] = 'k'
                    chess[myking[0]][myking[1]] = '.'
                    danger, mykingNew = getDanger(chess)
                    if danger[i][j] != 1:
                        canmove = True
                        break
                    chess = copy2d(oldChess)
