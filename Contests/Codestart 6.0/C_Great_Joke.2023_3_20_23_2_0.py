...
@bootstrap
def dfs(graph, node, vis, knows, knowsset):
    vis[node] = True
    for i in graph[node]:
        if not vis[i]:
            if i not in knowsset:
                knowsset.add(i)
                knows.append(i)
            yield dfs(graph, i, vis, knows, knowsset)
    yield
...
def main():
    ...
    for _ in range(TestCases):
        ...
        for q in range(n):
            m, time = [int(i) for i in input().split()]
            ...
            for meet in meets[time]:
                ...
                for knowingperson in knows:
                    if not vis[knowingperson]:
                        dfs(g, knowingperson, vis, knows, knowsset)
        
        print(*sorted([i+1 for i in knows]))
