    ...
    for _ in range(TestCases):
        ...
        def moveDiag(x, y, danger, chess):
            ...
            for dx in r:
                ...
                for dy in r:
                    if y + dy not in r:
                        break
                    # dsu.union(8 * x + y, 8 * (x + dx) + y + dy)
                    danger[x + dx][y + dy] = True
                    
                    if chess[x + dx][y + dy] not in ".k":
                        break
            for dx in r:
                if x - dx not in r:
                    break
        ...
        def moveHorVer(x, y, danger, chess):
            ...
            for dx in r:
                if x + dx not in r:
                    break
                # dsu.union(8 * x + y, 8 * (x + dx) + y)
                danger[x + dx][y] = True

                if chess[x + dx][y] not in ".k":
                    break
            for dx in r:
                if x - dx not in r:
                    break
            ...
            for dy in r:
                if y + dy not in r:
                    break
                # dsu.union(8 * x + y, 8 * x + y + dy)
                danger[x][y + dy] = True
                
                if chess[x][y + dy] not in ".k":
                    break
            for dy in r:
                if y - dy not in r:
                    break
