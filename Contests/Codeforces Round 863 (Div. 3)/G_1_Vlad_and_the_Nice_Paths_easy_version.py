...
def make_nCr_mod(max_n=2 * 10**4, mod=10**9 + 7):
...
def solve(st, en, arr, freq, k):
    ...
    for i in range(st, en + 1):
        for j in range(i, en + 1):
            if arr[i] != arr[j]:
                continue
            cnt = freq[j+1][arr[i]] - freq[i][arr[i]]
            if cnt < k:
                continue
            ans += nCr(cnt, k)
            ans += solve(st, i-1, arr, freq, k)
            ans += solve(j+1, en, arr, freq, k)
    return ans % M
...
def main():
    ...
    for _ in range(TestCases):
        ...
        # s = input()
        print()
        freq = [[0] * n for i in range(n+1)]
        for i in range(n):
            for j in range(n):
                freq[i+1][j] = freq[i][j] + (arr[i] == j)
        
        # for row in freq:
        #     print(row)
        
        print(solve(0, n-1, arr, freq, k))
