...
def main():
    ...
    for _ in range(TestCases):
        ...
        # s = input()
        given = [list(input()) for i in range(n)]
        # print(*given, sep='\n')
        drawing = [['.'] * m for _ in range(n)]
        ri = range(n)
        rj = range(m)

        for sz in range(k, k + max(n, m) + 1):
            for i in range(n):
                for j in range(m):
                    if given[i][j] == '.':
                        continue
                    ...
                    try:
                        for x in range(1, sz+1):
                            if i-x in ri and j-x in rj and j+x in rj and given[i-x][j-x] == given[i-x][j+x] == '*':
                                cnt += 2
                            else:
                                break
                    except:
                    ...
                    if cnt == 2 * sz + 1:
                        ...
                        drawing[i][j] = '*'
                        for x in range(1, sz+1):
                            drawing[i-x][j-x] = drawing[i-x][j+x] = '*'
        # for row in drawing:
        #     print(*row, sep='')
        if given == drawing:
            print("YES")
