...
signed main()
{
    ...
    while (TestCases--)
    {
        ...
        if (n == 2) {
            cout << ((k == 2) ? e9 : ((k == 1) ? max(arr[0], arr[1]) : min(arr[0], arr[1]))) << "\n";
            continue;
        }
        for (int i =0; i<k; i++) arr[i] = e9;
        
        sort(all(arr));

        cout << min(2 * arr[0], arr[1]) << endl;
        ...
        // }
        // else if (k == n - 1) {
        //     cout << min(2*arr[k], e9) << endl;
        // }
        // else {
        //     cout << min(2*arr[k], arr[k+1]) << endl;
        // }
