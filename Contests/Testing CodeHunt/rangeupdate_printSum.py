...
class SegmentTree:
    def __init__(self, data, default=(INF, INF), func=min):
...
def main():
    ...
    for _ in range(TestCases):
        ...
        # s = input()
        d = [(-INF, i) for i in range(n)]
        seg = SegmentTree(d)
        seg2 = SegmentTree(a, default=0, func=lambda x,y:x+y)

        for qq in range(q):
            l, r = [int(i) - 1 for i in input().split()]
            while 1:
                v, idx = seg.query(l, r + 1)
                if v != -INF:
                    break
                seg[idx] = (1, idx)
                seg2[idx] = k
        
            print(seg2.query(0, n))
