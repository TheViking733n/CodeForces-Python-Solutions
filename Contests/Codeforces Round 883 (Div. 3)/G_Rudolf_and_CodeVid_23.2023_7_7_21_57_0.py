...
def dijkstra(graph, start):
    ...
    queue = SortedList([encode(0, start)])
    while queue:
        # path_len, v = decode(heappop(queue))
        path_len, v = decode(queue.pop())
        if path_len == dist[v]:
            for w, edge_len in graph[v]:
                if edge_len + path_len < dist[w]:
                    dist[w], parents[w] = edge_len + path_len, v
                    # heappush(queue, encode(edge_len + path_len, w))
                    queue.add(encode(edge_len + path_len, w))
    return dist, parents
...
for _ in range(int(input())):
    ...
    init = inp()
    medi = [(int(input()), inp(), inp()) for _ in range(m)]
    mask = (1 << n) - 1
    mat = [[oo] * (mask + 1) for _ in range(mask + 1)]
    for u in range(mask + 1):
        for d, c, e in medi:
            v = (u & (mask ^ c)) | e
            mat[u][v] = min(mat[u][v], d)
    
    g = [[] for _ in range(mask + 1)]
    for u in range(mask + 1):
        for v in range(mask + 1):
            if mat[u][v] == oo: continue
            g[u].append((v, mat[u][v]))
    
    dist, parents = dijkstra(g, init)
