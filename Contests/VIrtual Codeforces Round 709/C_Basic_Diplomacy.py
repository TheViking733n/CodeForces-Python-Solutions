...
def solve():
    ...
    for i in range(m):
        if ans[i] == -1:
            if count.get(mx, 0) < (m + 1) // 2 and mx in v[i]:
                ans[i] = mx
                count[mx] += 1
            else:
                for j in range(len(v[i])):
                    if v[i][j] != mx and count.get(v[i][j], 0) < (m + 1) // 2:
                        ans[i] = v[i][j]
                        count[v[i][j]] = count.get(v[i][j], 0) + 1
                        break
