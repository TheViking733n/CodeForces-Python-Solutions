...
class RangeQuery:
    def __init__(self, data, func=lambda x,y:gcd(x, y)):
        self.func = func
    ...
    for _ in range(TestCases):
        ...
        for i in range(n):
            ...
            while low <= high:
                ...
                mid = (low + high) // 2
                try:
                    g1 = rgq.query(i, mid + 1)
                    g2 = 0 if mid + 2 > n else rgq.query(i, mid + 2)
                except:
                    print("Error")
                if g1 > k and g2 <= k:
                    cur = mid
                    ...
                    high = mid - 1
            # print(rgq.query(i, cur + 1), arr[i:cur + 1]) if cur >= i else print(-1, arr[i:])
            ans += cur - i + 1
        print(n * (n + 1) // 2 - ans)
