...
def count_leaf_nodes(root, graph, leafs):
    visited = set()
    dfs(root, graph, visited, leafs)
...
@bootstrap
def dfs(node, graph, visited, leafs):
    visited.add(node)
    ...
    for i in graph[node]:
        if i not in visited:
            leafs[node] += yield dfs(i, graph, visited, leafs)
    yield leafs[node]
...
def main():
    ...
    for _ in range(TestCases):
        # n = int(input())
        # n = int(input())
        # patterns = [int(i) for i in input().split()]
        ...
        ans = [0]*n
        count_leaf_nodes(0, graph, ans)
        # print(ans)
        q = int(input())
        for i in range(q):
            query = [int(i) for i in input().split()]
            print(ans[query[0]-1]*ans[query[1]-1])
