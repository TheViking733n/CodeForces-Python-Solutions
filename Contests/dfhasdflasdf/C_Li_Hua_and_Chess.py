...
def main():
    ...
    for _ in range(TestCases):
        ...
        if n == 1:
            moves = query(1, 1)
            print("!", 1, moves + 1, flush=True)
            sys.stdout.flush()
            continue

        if m == 1:
            moves = query(1, 1)
            print("!", moves + 1, 1, flush=True)
            sys.stdout.flush()
            continue
        ...
        if d3 == 0:
            print("!", x, y, flush=True)
            continue
        if d3 != 0 and x1 != x2 and y1 != y2:
            x, y = x1 + x2 - x, y1 + y2 - y
            print("!", x, y, flush=True)
            continue
        
        if x1 == x2:
            x2, y2 = x1 + x2 - x, y1 + y2 - y
            diry = 1 if (y2 - y >= 0) else -1
            y += diry * d3
        elif y1 == y2:
            x2, y2 = x1 + x2 - x, y1 + y2 - y
            dirx = 1 if (x2 - x >= 0) else -1
            x += dirx * d3
