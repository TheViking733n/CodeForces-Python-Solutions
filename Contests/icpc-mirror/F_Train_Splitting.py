...
def dfs(u, vis, g, company, k, cnt):
    vis[u] = True
    for v in g[u]:
        if not vis[v] and (u, v) not in company:
            n = len(g)
            cnt.add(v)
            cnt.add(u)
...
def main():
    ...
    for _ in range(TestCases):
        ...
        cnt = {0}
        while 1:
            temp = set(edges) - set(company.keys())
            if not temp:
                break
            u, v = list(temp)[0]
            dfs(u, vis, g, company, 1, cnt)

        # print(company)
        for u, v in company:
            if u < v:
                print((u+1, v+1), company[(u, v)])

        print(*[company[e] for e in edges])
