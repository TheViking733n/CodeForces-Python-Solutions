...
def main():
    ...
    for _ in range(TestCases):
        ...
        for i in range(n):
            ...
            match1 = match2 = 0
            if i1 in range(n):
                for j in range(min(len(arr[i][0]), len(arr[i1][0]))):
                    if arr[i][0][j] == arr[i1][0][j]:
                        match1 += 1
                    else:
                        break
            if i2 in range(n):
                for j in range(min(len(arr[i][0]), len(arr[i2][0]))):
                    if arr[i][0][j] == arr[i2][0][j]:
                        match2 += 1
                    else:
                        break
            ans.append((max(match1, match2), arr[i][1]))
        ans.sort(key=lambda x: x[1])
        print(*[i[0] for i in ans], sep='\n')
