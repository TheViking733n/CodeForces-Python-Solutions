...
def main():
    ...
    for _ in range(TestCases):
        ...
        for i in leaf:
            # if dsu.find(i) == start:
            #     continue
            if dsu.find(i) not in counted:
                more += dsu.set_size(i)
                counted.add(dsu.find(i))
        ...
        # ans = sm
        # for i in range(n):
        #     if valid1[i] == 1 and i not in leaf:
        #         ans -= 1
        # for i in range(sz, len(valid)):
        #     sm += valid[i] - valid[i - sz]
        #     ans += sm
