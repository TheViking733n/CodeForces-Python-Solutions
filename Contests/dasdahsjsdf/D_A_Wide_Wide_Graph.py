...
def findDiameter(tree, n):
 
    init = bfs(1, tree, n)
    val = bfs(init, tree, n)
    return diameter[val], init, val


def getNodesArr(tree, start):
    n = len(tree) - 1
    visited = [0 for i in range(n + 1)]
    q = deque()
    q.append(start)
...
def main():
    ...
    for _ in range(TestCases):
        ...
        dia, a, b = findDiameter(tree, n)
        # print(dia, a, b)

        left = getNodesArr(tree, a)
        right = getNodesArr(tree, b)
