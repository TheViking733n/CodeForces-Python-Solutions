...
def dfs(u, p, tree, par):
    par[u] = p
    for v in tree[u]:
        if v != p:
            dfs(v, u, tree, par)
    
def dfsFlow(u, p, val, tree):
    if p != -1: val[u] += val[p]
    for v in tree[u]:
        if v != p:
            dfsFlow(v, u, val, tree)
...
def main():
    ...
    for _ in range(TestCases):
        n = int(input())
        edge = [tuple(int(i)-1 for i in input().split()) for j in range(n-1)]
