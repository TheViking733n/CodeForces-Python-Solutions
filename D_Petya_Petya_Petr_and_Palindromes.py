...
def f(arr, k, N):
    ...
    for i in range(1, len(arr)):
        if arr[i] <= k0: continue
        x, y = arr[i] - k0, N - arr[i]
        low, high = max(2, k - (2 * y - 1)), min(k - 1, 2 * x)
        cnt += bisect_right(arr, arr[i] - low) - bisect_left(arr, arr[i] - high)
    return cnt
...
def solve(arr, k, N):
    ...
    for i in range(len(arr)):
        if arr[i] & 1: od.append(arr[i])
        else: ev.append(arr[i])
    return f(ev, k, N) + f(od, k, N)
