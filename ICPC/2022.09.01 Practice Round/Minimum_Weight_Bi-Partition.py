...
def dfs(u, g, vis, cc):
    ...
    for v in g[u]:
        if not vis[v]:
            yield dfs(v, g, vis, cc)
    yield

@bootstrap
def dfs2(u, g, col, c, ans):
    col[u] = c
    for v in g[u]:
        if col[v] == -1:
            ans.append((u, v))
            yield dfs2(v, g, col, 1-c, ans)
    yield
...
def main():
    ...
    for _ in range(TestCases):
        ...
        for i in range(len(cc)):
            for node in cc[i]:
                aci.append((val[node], i, node))
        
        aci.sort()
        # print(aci)
        a2 = []

        for i in range(len(aci) - 1):
            if aci[i][1] != aci[i+1][1]:
                a2.append((abs(aci[i][0]-aci[i+1][0]), aci[i][2], aci[i+1][2]))
