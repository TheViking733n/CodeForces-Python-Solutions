...
def whereYouWantToGo(l, i, r, a):
    n = len(a)
    if i == 0: return 1
    if i == n-1: return 0
    return int(a[r] + a[l] < 2 * a[i])

def good(i, arr):
    n = len(arr)
    assert 1 <= i < n-1
    x = whereYouWantToGo(i-2, i-1, i, arr) << 2
    y = whereYouWantToGo(i-1, i, i+1, arr) << 1
    z = whereYouWantToGo(i, i+1, i+2, arr)
    print(f"{x>>2} {y>>1} {z}".replace('1', '->').replace('0', '<-'))
    return x + y + z in [2, 5]
    ...
    for _ in range(TestCases):
        ...
        for i in range(n - 1):
            for j in range(i + 1, n):
                d = arr[j] - arr[i]
                left = bisect_left(arr, arr[i] - d)
                right = bisect_left(arr, arr[j] + d)
                # ans += powm(2, left + n - right)
                ans += pow2[left + n - right]
                ans %= M
        print(ans)
