...
# def dfs(pa1, pa2, graph, visited, ans):
#     visited[pa1] = True
#     for ch in graph[pa1]:
#         if BS(graph[ch], pa2) != -1:
#             ans[0] = min(ans[0], len(graph[ch])+len(graph[pa1])+len(graph[pa2])-6)
#         if not visited[ch]:
#             dfs(ch, pa1, graph, visited, ans)


# def dfs(node, graph, visited, vis, cycle, ans):
#     visited[node] = True
#     vis[node] = True
#     cycle.append(node)
#     for ch in graph[node]:
#         if vis[ch]:
#             ans[0] = min(ans[0], len(cycle)-1)
#             print([i+1 for i in cycle])
#         if not visited[ch]:
#             dfs(ch, graph, visited, vis, cycle, ans)
#         vis[node] = False
#     cycle.pop()
#     vis[node] = False
...
def main():
    ...
    for _ in range(TestCases):
        ...
        for node in range(n):
            ln = len(graph[node])
            for i1 in range(ln):
                for i2 in range(i1+1, ln):
                    ch1, ch2 = graph[node][i1], graph[node][i2]
                    if BS(graph[ch1], ch2) != -1:
                        ans = min(ans, len(graph[ch1])+len(graph[ch2])+len(graph[node])-6)
