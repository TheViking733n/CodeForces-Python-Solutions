...
def main():
    ...
    for _ in range(TestCases):
        ...
        n = int(input())
        arr = [tuple(int(i) for i in input().split()) for i in range(n)]
        arr.sort()
        # print(arr)
        ...
        for jj in range(1):
            ...
            for i in range(n):
                cur = b[i]
                # idx = bisect_left(a, cur) + 1
                # for loop in range(5):
                #     if idx != i and 0 <= idx < n:
                #         amax = a[idx]
                #         bmax = cur
                #         if idx + 1 < n:
                #             bmax = max(bmax, sufmax[idx + 1])
                #         ans = min(ans, abs(amax - bmax))
                #     idx -= 1
                # idx = bisect_left(a, cur+1) + 1
                # for loop in range(5):
                #     if idx != i and 0 <= idx < n:
                #         amax = a[idx]
                #         bmax = cur
        ...
        #     idx = i + 1
        #     if idx < n:
        #         if abs(sufmax[idx] - cur) > ans:
        #             # if b[i] not in lst:
        #             lst.add(b[i])
        #             continue
        #         ans = abs(sufmax[idx] - cur)
        #     if lst:
        #         idx = lst.bisect_left(cur)
        #         try: ans = min(ans, abs((max(sufmax[i+1], lst[idx]) if i+1 < n else lst[idx]) - cur))
        #         except: pass
        #         if idx > 0:
        #             try: ans = min(ans, abs((max(sufmax[i+1], lst[idx-1]) if i+1 < n else lst[idx-1]) - cur))
        #             except: pass
        #     # ans = min(ans, a2[cur])
        #     # if b[i] not in lst:
        #     lst.add(b[i])
