...
def main():
    ...
    """
    Given array arr, find no. of ways to make sum k
    f(i, left) : no. of ways to make sum left using arr[1..i]
    f(i, left)   = f(i-1, left) + f(i-1, left-1) + f(i-1, left-2) + ... + f(i-1, left-arr[i])
    f(i, left-1) = f(i-1, left-1) + f(i-1, left-2) + f(i-1, left-3) + ... + f(i-1, left-arr[i])
    Therefore, f(i, left) = f(i-1, left) + f(i, left-1)
    """
