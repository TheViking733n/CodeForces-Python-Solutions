...
def main():
    ...
    for _ in range(TestCases):
        ...
        lar = defaultdict(int)
        for i in range(n):
            c, d, h = [int(i) for i in input().split()]
            # val.append(h * d)
            # coin.append(c)
            lar[c] = max(lar[c], h * d)
        m = int(input())
        ...
        distintCoins = sorted(lar.keys())
        best = [0] * (k + 1)
        for c in distintCoins:
            vi = lar[c]
            best[c] = max(best[c], vi)
            for j in range(2, k + 1):
                if c * j > k: break
                best[c * j] = max(best[c * j], vi * j)
        for i in range(1, k+1):
            best[i] = max(best[i], best[i-1])
        
        ans = []
        for vm in valm:
            idx = bisect_right(best, vm)
            ans.append(-1 if idx > k else idx)
        print(*ans)
        ...
        #     mincost = oo
        #     for i in range(n):
        #         x = coin[i] * (1 + int((vm / val[i])))
        #         # print(val[i], coin[i], x)
        #         mincost = min(mincost, x)
        #     ans.append(-1 if mincost > k else mincost)
        # print(*ans)
