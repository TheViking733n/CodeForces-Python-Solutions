...
def main():
    ...
    for _ in range(TestCases):
        ...
        n = int(1e6)+5
        arr = [A + B * i for i in range(n)]
        ps = [0] + list(accumulate(arr))
        # print(arr)
        
        for _ in range(q):
            l, t, m = [int(i) for i in input().split()]; l -= 1
            if l >= n or arr[l] > t:
                print(-1)
                continue
            # r = l; sm = 0
            # while r < n and arr[r] <= t and sm + arr[r] <= m * t:
            #     sm += arr[r]
            #     r += 1
            # r -= 1
            r1 = bisect_right(arr, t, lo=l) - 1
            r2 = bisect_right(ps, m * t + ps[l], lo=l) - 2
            print(min(r1, r2) + 1)
