...
# def dfs(u, par, g):
#     S = C = ANS = sigma_a = sigma_a_sq = 0
#     for v in g[u]:
#         if v == par: continue
#         s1, c1, ans1 = dfs(v, u, g)
#         a = s1 + c1
#         C += c1; ANS += ans1
#         sigma_a += a; sigma_a_sq += a * a
#     S = sigma_a
#     C += 1
#     ANS += (sigma_a ** 2 - sigma_a_sq) // 2 + S
#     print(u, S, C, ANS)
#     return S, C, ANS
...
@bootstrap
def dfs(u, par, g, size, dep=0):
    ans = 0
    size[u] = 1
    for v in g[u]:
        if v == par: continue
        ans += yield dfs(v, u, g, size, dep + 1)
        size[u] += size[v]
    yield ans + dep

@bootstrap
def dfs1(u, par, g, size, dp):
    n = len(g)
    for v in g[u]:
        if v == par: continue
        dp[v] = (dp[u] + n - 2 * size[v]) % M
        yield dfs1(v, u, g, size, dp)
    yield
