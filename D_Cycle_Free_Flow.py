...
def dfs(u, pa, g, par, mx, W, dep):
    par[u][0] = pa
    mx[u][0] = W[(u, pa)]
    for i in range(1, 20):
        par[u][i] = par[par[u][i-1]][i-1]
        mx[u][i] = max(mx[u][i-1], mx[par[u][i-1]][i-1], mx[u][i])
    for v in g[u]:
        if v != pa:
            dep[v] = dep[u] + 1
            dfs(v, u, g, par, mx, W, dep)


def LCA(u, v, par, mx, dep):
    if dep[u] < dep[v]:
        u, v = v, u
...
def query(u, v, par, mx, dep):
    lca = LCA(u, v, par, mx, dep)
    ans = 0
    for i in range(19, -1, -1):
...
def main():
    ...
    for _ in range(TestCases):
        ...
        mx = [[0]*20 for i in range(n)]

        dep = [0]*n

        dfs(0, 0, g, par, mx, W, dep)
        print(par)
        print(mx)

        for _ in range(qq):
            u, v = [int(i)-1 for i in input().split()]
            print(query(u, v, par, mx, dep))
