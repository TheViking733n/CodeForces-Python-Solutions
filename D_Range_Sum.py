...
def main():
    ...
    for _ in range(TestCases):
        ...
        idx = bisect_left(triangular, delta)
        remove = triangular[idx] - delta
        arr = [i for i in range(1, idx + 1) if i != remove]
        if not remove and len(arr) > 1:
            arr.append(arr.pop(0) + arr.pop())
        arr = [a] + [a + i for i in arr]
        arr.append(a + 2 * n)
        ...
        #     # print(delta)
        #     idx = bisect_left(triangular, delta)
        #     # print(idx, triangular[idx])
        #     remove = None
        #     if triangular[idx] != delta:
        #         remove = triangular[idx] - delta
        #     arr = [i for i in range(1, idx + 1) if i != remove]
        #     if not remove and len(arr) > 1:
        #         arr.append(arr.pop(0) + arr.pop())
        #     arr = [a + i for i in arr]
        #     if arr and arr[-1] >= a + root:
        #         low = mid + 1
        #         continue
        #     print(a, *arr, a + root)
        #     # print(a + root)
        ...
        # if n % 2 == 0:
        #     print(*list(range(n//2,n)),*list(range(n+1,3*n//2+1)))
        #     continue
        # ans = list(range((n+1)//2+2,(n+1)//2+2+n))
        # ans[0] -= 1; ans[-2] += 1; ans[-1] += 1
        # print(*ans)
