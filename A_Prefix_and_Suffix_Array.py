...
def main():
    ...
    for _ in range(int(input())):
        ...
        x1, x2 = [i for i in arr if len(i) == 1]
        s1, s2 = [i for i in arr if len(i) == n - 1]
        freq = Counter(x1 + x2 + s1 + s2)
        for k in freq: freq[k] >>= 1
        if Counter(s1 + x1) != freq: x1, x2 = x2, x1
        ans = (s1 + x1) if (s1 + x1 == x2 + s2) else (x1 + s1)
