...
def getMask(s):
    return sum(1 << (ord(i) - ord('A')) for i in s)
    ...
    for _ in range(TestCases):
        ...
        for _ in range(n):
            ...
            for mask in range(8):
                ...
                dp2[mask | m] = min(dp2[mask | m], dp[mask] + x)
            dp = dp2
        ...
        #         if len(set(arr[i] + arr[j])) == 3:
        #             ans = min(ans, p[arr[i]] + p[arr[j]])
        #         for k in range(j+1, n):
        #             if len(set(arr[i] + arr[j] + arr[k])) == 3:
        #                 ans = min(ans, p[arr[i]] + p[arr[j]] + p[arr[k]])
