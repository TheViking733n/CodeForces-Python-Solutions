...
def main():
    ...
    for _ in range(TestCases):
        ...
        ans = n // 2
        for sm in range(2 * k + 2):
            change = n - ps[sm] - freq[sm]
            ans = min(ans, change)
        print(ans)
...
def main1():
    ...
    for _ in range(TestCases):
        ...
        mins = []
        maxs = []
        cnt = [[0, i] for i in range(2 * k + 1)]
        for i in range(n//2):
            a, b = arr[i], arr[n-1-i]
            a, b = arr[i], arr[n-1-i] = sorted([a, b])
            sm = a + b
            mins.append(a + 1)
            maxs.append(b + k)
            cnt[sm][0] += 1
        ...
        ans = n//2
        for freq, sm in cnt:
            # if freq <= maxfreq-int(n**.5)-2: break
            if freq == 0: break
            left = bisect_right(maxs, sm-1)
            right = n//2 - bisect_left(mins, sm+1)
            change = 2 * left + 2 * right + (n//2 - left - right - freq)
            # print(freq, sm, left, right, change)
