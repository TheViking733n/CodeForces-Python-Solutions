...
def minSwaps(arr):
    ...
    for i in range(n):
        ...
        if cur == i or vis[i]:
            continue
        cnt = 0
        first_swap_idx = idx[cur]
        swapped_idx = [idx[cur]]
        while not vis[cur]:
            vis[cur] = 1
            v1, v2 = cur, arr[cur]
            swapped_idx.append(v2)
            # lastSwap = (idx[v1], idx[v2])
            # if max(lastSwap) - min(lastSwap) == 1:
            #     adjSwap = lastSwap
            # print(v1+1, v2+1, idx[v1]+1, idx[v2]+1)
            idx[v1], idx[v2] = idx[v2], idx[v1]
            cur = arr[cur]
            cnt += 1
        swaps += cnt - 1
        # lastSwap = (idx[v1], first_swap_idx)
        # if max(lastSwap) - min(lastSwap) == 1:
        #     adjSwap = lastSwap
        if adjSwap: continue
        swapped_idx.sort()
        ln = len(swapped_idx)
        for j in range(ln):
            if abs(swapped_idx[j] - swapped_idx[(j+1)%ln]) == 1:
                adjSwap = (swapped_idx[j], swapped_idx[(j+1)%ln])
                break
    return swaps, adjSwap
...
def main():
    ...
    for _ in range(TestCases):
        ...
        alreadySwapped = 0
        for i in range(n):
            if arr[i] == i - 1 or arr[i] == i + 1:
                idx = arr.index(i)
                arr[i], arr[idx] = arr[idx], arr[i]
                alreadySwapped = 1
                break
        
        ans, adjSwap = minSwaps(arr)
        # print(adjSwap) 
        if alreadySwapped:
            print(ans)
        elif adjSwap:
