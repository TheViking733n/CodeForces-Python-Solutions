...
def main():
    ...
    for _ in range(TestCases):
        ...
        ind = [myDict(list), myDict(list)]
        for i in range(n + 1):
            ind[i & 1][px[i]].append(i)
        ...
        for _ in range(q):
            ...
            if px[r] != px[l - 1]:
                print(-1)
                continue
            # Now not all elements in [l, r] are zero but xor is zero
            if r - l + 1 & 1 or arr[l - 1] == 0 or arr[r - 1] == 0:
                print(1)
                continue
            # Now xor is zero and number of elements is even
            pre = px[l - 1]
            indices = ind[l & 1][pre]
            st = bisect_left(indices, l)
            en = bisect_right(indices, r) - 1
            if st > en:
                print(-1)
            else:
                print(2)
...
class myDict():
    ...
    def __repr__(self) -> str:
        return str({self.RANDOM^k:v for k,v in self.dict.items()})
