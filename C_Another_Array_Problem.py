...
def maximize(arr, st, en, ps, rmn, rmx, delta):
    if st >= en:
        return

    unchanged = good = (-INF, 0, 0, 0)
    for i in range(st, en + 1):
        mx1, i1 = rmx.query(st, i + 1)# if i > st else (arr[i], i)
        mn1, i2 = rmn.query(i, en + 1)
        mn2, j1 = rmn.query(st, i + 1)# if i > st else (arr[i], i)
        mx2, j2 = rmx.query(i, en + 1)

        v1 = abs(mx1 - mn1)
        v2 = abs(mx2 - mn2)

        delta1 = v1 * (i2 - i1) - (ps[i2+1] - ps[i1])
        delta2 = v2 * (j2 - j1) - (ps[j2+1] - ps[j1])

        if delta1 > 0:
            good = max(good, (delta1, -(i2 - i1), i1, i2))
        if delta2 > 0:
            good = max(good, (delta2, -(j2 - j1), j1, j2))
    
    print(st, en, good)
    
    if good is unchanged:
        return
    ...
    maximize(arr, st, i1, ps, rmn, rmx, delta)
    maximize(arr, i2, en, ps, rmn, rmx, delta)
...
def main():
    ...
    for _ in range(TestCases):
        ...
        delta = [0]
        maximize(arr, 0, n - 1, ps, rmn, rmx, delta)
