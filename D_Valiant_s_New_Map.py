...
def main():
    ...
    for _ in range(TestCases):
        ...
        # sg = [RangeQuery(row) for row in mat]
        ...
        for i in range(n):
            ...
            for j in range(m):
                l = mat[i][j]
                j2 = min(j + l, m)
                low, high = ans, min(l, j2) + 1
                h = 0
                while low < high:
                    mid = (low + high) // 2
                    if sg.query(j, min(m, j + mid)) >= mid:
                        h = max(mid, h)
                        low = mid + 1
                    else:
                        high = mid
        ...
        # sg = [RangeQuery(row) for row in height]
        for i in range(m):
            sg = RangeQuery(height[i])
            for j in range(n):
                l = height[i][j]
                j2 = min(j + l, n)
                low, high = 0, l + 1
