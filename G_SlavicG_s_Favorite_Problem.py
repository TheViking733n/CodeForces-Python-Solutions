...
@bootstrap
def dfs(node, par, target, val, g, st: set):
    # if not par: val ^= g[node][par][1]
    for child, w in g[node]:
        if child == par or child == target:
            continue
        val ^= w
        st.add(val)
        yield dfs(child, node, target, val, g, st)
        val ^= w
    # if not par: val ^= g[node][par][1]
    yield

@bootstrap
def dfsFind(node, par, target, val, g, ans):
    # if not par: val ^= g[node][par][1]
    if node == target:
        ans[0] = val
        yield
    for child, w in g[node]:
        if child == par:
            continue
        val ^= w
        yield dfsFind(child, node, target, val, g, ans)
        val ^= w
    # if not par: val ^= g[node][par][1]
    yield
...
def main():
    ...
    for _ in range(TestCases):
        ...
        n, a, b = [int(i) - 1 for i in input().split()]
        n += 1
        # s = input()
