...
def main():
    ...
    for _ in range(TestCases):
        ...
        # s = input()
        ind_lst = [[] for i in range(n)]

        for i in range(n):
            ind_lst[arr[i]].append(i)
        ...
        for i in range(n):
            ...
            #     else:
            #         diff.append(bw)
            # ans[i] = max(diff.count(0) - diff.count(1), 1)

            even = odd = 1
            for j in range(1, len(ind_lst[i])):
                if not (ind_lst[i][j] - ind_lst[i][j-1])&1:
                    even += 1
                else:
                    odd += 1
            ans[i] = even
