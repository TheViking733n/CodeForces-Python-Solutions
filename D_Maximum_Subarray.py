...
def segfunc(n1, n2):
    mx = max(n1[0], n2[0], n1[2] + n2[1])
    pre = max(n1[1], n1[3] + n2[1])
    suf = max(n2[2], n1[2] + n2[3])
    sm = n1[3] + n2[3]
    return (mx, pre, suf, sm)

class SegmentTree:
    def __init__(self, data, default=(0, 0, 0, 0), func=segfunc):
...
def main():
    ...
    for _ in range(TestCases):
        ...
        seg = SegmentTree([(i, i, i, i) for i in arr])
        ans = seg.query(0, n)[0]

        for i in range(k, n):
            arr[i] += 2*x
            seg[i] = (arr[i], arr[i], arr[i], arr[i])
            arr[i-k] -= 2*x
            seg[i-k] = (arr[i-k], arr[i-k], arr[i-k], arr[i-k])
            ans = max(ans, seg.query(0, n)[0])
