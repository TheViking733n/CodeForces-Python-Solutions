...
@bootstrap
def f(st, en, h, arr:list):
    # print(st, en, h)
    if st > en:
        yield 0
    if st == en:
        yield min(1, arr[st] - h)
    # mn = min(arr[st:en + 1])
    # ans = mn - h
    # st2 = st
    ...
    #         st = i + 1
    # ans += f(st, en, mn, arr)
    # print('=>', st, en, ans, en - st + 1)

    # mn = min(arr[st:en + 1])
    # idx = arr.index(mn, st, en + 1)
    global rmq
    mn, idx = decode(rmq.query(st, en + 1))
    ans = mn - h
    ans += yield f(st, idx - 1, mn, arr)
    ans += yield f(idx + 1, en, mn, arr)

    yield min(ans, en - st + 1)
...
def main():
    ...
    for _ in range(TestCases):
        ...
        # s = input()
        global rmq
        rmq = RangeQuery([encode(arr[i], i) for i in range(n)])
        print(f(0, n - 1, 0, arr))
