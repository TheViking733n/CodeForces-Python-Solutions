...
# def powmStr(num, powerstr, mod):
#     ans = 1
#     num %= mod
#     for i in range(len(powerstr) - 1, 1, -1):
#         power = powerstr[i]
#         if power == '1': ans = (ans * num) % mod
#         num = (num * num) % mod
#     return ans


def subtractOne(num: list):
    i = len(num) - 1
    while i >= 0:
        if num[i] == 0: num[i] = 9
        else: break
        i -= 1
    num[i] = int(num[i]) - 1
    if num[0] == 0: num.pop(0)
    return num

def baseMod(val, exp, mod):
    ans = 0
    x = 1
...
def decimalExponentiation(base, power, mod):
    ans = 1
    for i in range(len(power) - 1, -1, -1):
        for _ in range(power[i]):
            ans = (ans * base) % mod
        base = powm(base, 10, mod)
    return ans

def main():
    b, n, mod = input().split(); mod = int(mod)
    base = baseMod(b, 10, mod)
    n = [int(i) for i in n]
    subtractOne(n)
    v1 = decimalExponentiation(base, n, mod)
    v2 = (base - 1) % mod
    ans = (v1 * v2) % mod
    print((ans - 1) % mod + 1)
